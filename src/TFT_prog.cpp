...
static errorState_t drawButtons(TFT_eSPI *tft,screen_t * screen)
{
    ...
    {  
        ...
        tft->fillRect(screen->drawnButton->button_x,screen->drawnButton->button_y,screen->drawnButton->button_w,screen->drawnButton->button_h,screen->drawnButton->button_color);

        tft->setTextColor(screen->drawnButton->button_txt_color);
        ...
        tft->drawString(screen->drawnButton->button_text, screen->drawnButton->button_x + (screen->drawnButton->button_w / 2),  screen->drawnButton->button_y + (screen->drawnButton->button_h / 2));

        screen->drawnButton->chosen = FALSE ;
...
errorState_t TFT_drawScreen(screen_t * screen,TFT_eSPI * tft)
{
    ...
    switch(screen->screenMode){
        ...
        case VERFIY_SCREEN_DONE   :
        {
            ...
            tft->setTextSize(screen->screenTxtSize);
            tft->println("HI");
            tft->println("Your device is reay ");
            break;
        }
...
errorState_t TFT_getScreenButtonState(screen_t * screen )
{
    ...
    for(counter = 0 ; counter < screen->numberOfButtons ; counter++)
    {  
        if(screen->drawnButton->chosen == TRUE)
        {
              screen->active_button = screen->drawnButton ;
              return DONE;
        }
        screen->drawnButton++;
    }
